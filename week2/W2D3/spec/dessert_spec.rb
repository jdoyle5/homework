...
describe Dessert do
  let(:chef) { double("chef", name: "Joey") }
  let(:pie) { Dessert.new("pie", 25, chef) }
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new("pastry", "bazillion", chef) }.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do
      ...
      ingredients = ["apples", "dough", "sugar", "cinnamon"]
  ...
  describe "#eat" do
    ...
    it "raises an error if the amount is greater than the quantity" do
      expect { pie.eat(500) }.to raise_error("not enough left!")
    end
  end

  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Joey the Great Baker")
      expect(pie.serve).to eq("Chef Joey the Great Baker has made 25 pies!")
    end
